        ...
        if (demoData.products && demoData.products.length > 0) {
            ...
            if (error) {
                if (_productExpiryColumnAvailable && isMissingProductExpiryColumn(error)) {
                    ...
                    console.warn('⚠️ Supabase products table is missing expiry_date column. Run: ALTER TABLE products ADD COLUMN expiry_date DATE;')
                    const fallbackData = productsData.map(({ expiry_date, ...rest }) => rest)
                    const { error: fallbackError } = await supabase
                        .from('products')
                        .upsert(fallbackData, { onConflict: 'code' })
